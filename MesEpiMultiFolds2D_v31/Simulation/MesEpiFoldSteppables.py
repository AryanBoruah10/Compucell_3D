...
class FetchCoordsandApplyForces(SteppableBasePy):
    ...
    def muscle_coordinates(self):
                ...
                num_fold = (self.dim.x - self.start_fold_coord)//self.fold_seperation      #FLOOR DIVISION TO IGNORE DECIMALS
                num_fold = num_fold + 1                  # FLOOR DIV CONVERT FOR EX 1.5 TO 1, SO +1 TO INCREMENT THE NUMBER
                ...
                for temp in muscle_x_coords: 
                    ...
                    result = temp + 50 
                    '''muscle_to_fold [174, 224, 524, 574]
                       muscle_x_coords [174, 524]'''    
                ...
                print("muscle_to_fold", muscle_to_fold)
                print("muscle_x_coords", muscle_x_coords)
    ...
    def step(self, mcs):
        ...
        '''
        temp = self.muscle_fold_coordinates                 # This is how temp looks like::::: [174, 224, 524, 574] #
        #print("This is how temp looks like:::::", temp)
        ...
        if mcs%100==0 and mcs<4000:
                ...
                for cell in self.cell_list_by_type(self.ECM):
                        ...
                        for first, last in zip(temp[::2], temp[1::2]):
                                                       
                            if first <= cell.xCOM <= last:
        ...
        if mcs>=4000 and mcs%100==0:
                ...
                fold_centers = self.muscle_fold_coordinates             # X POSITIONS OF THE TWO FOLD CENTERS. USING MUSCLE CORRDINATES TO FOLD THE STROMAL LAYER. HERE FOLD CENTER COARSELY MEANS MUSCLES ITSELF 
